...
The shell supplies Bash, Python, Ruff, ShellCheck, jq, Actionlint, Zizmor, Markdownlint, Git, and GNU utilities. `repo-check` checks Markdown using the shared fleet rule set, lints development helpers, runs adapter regressions, and executes the existing pre-push checks for managed shell/JSON files and workflows. There is no application unit-test suite. Rendered profile review, external links, images, badges, and attribution still need the review described in [RELEASING.md](../RELEASING.md).
...
Apple execution is not verified by Linux tests. The environment validates source and tooling. It does not render the GitHub profile or verify external links and images. Review those separately when changing the profile.
